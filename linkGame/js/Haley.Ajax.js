 ...
 *
 *   Release 2.0.0
 *   Author: Haley Wang<whlok@126.com>
 *   License: http://www.gnu.org/licenses/gpl.html GPL
 *   Copyright (c) 2008, HaleyWang, All Rights Reserved
 */
...
Haley.Ajax.prototype = 
{
    ...
    start: function()
    {
        ...
        if(this.options.timeout && typeof(this.options.timeout) == "number")
            this.timeout = setTimeout(function(){
                                                instance.onAbort();
                                                instance.onLoaded();
                                                instance.onTimeout();}, this.options.timeout * 1000);

        this.xmlhttp.send( this.method == "post" ? (this.postdata || requestParams) : null);
        if(!this.async) this.dispatchEvent();
    },
    getXmlHttpObject: function()
    {
        ...
        {
            var msxmls = new Array( "Msxml2.XMLHTTP",
                                     "Microsoft.XMLHTTP");
    ...
    dispatchEvent: function()
    {
        if( this.xmlhttp.readyState == 4 )
        {
            ...
            if( this.xmlhttp.status == 200 )
                this.onComplete({textString: this.xmlhttp.responseText,
                                 xmlDocument: this.xmlhttp.responseXML});
            else if( this.xmlhttp.status == 0 )
                this.onAbort();
            else
                this.onError({url: this.url,
